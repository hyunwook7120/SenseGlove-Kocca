...
feedback_call_count = 0  # 피드백 함수 호출 횟수 추적
...
def show_time_window():
    ...
    button_1 = tk.Button(time_window, text="10 seconds", width=20, command=lambda: submit_time(9))
...
def show_feedback_window():
    global feedback_entry, feedback_window, feedback_call_count
    feedback_call_count += 1  # 호출될 때마다 카운트 증가
    
    feedback_window = tk.Tk()
    feedback_window.title("Feedback")
    
    # 호출 횟수 표시
    label_count = tk.Label(feedback_window, text=f"Feedback Window Call: {feedback_call_count}")
    label_count.pack(pady=10, anchor='w')  # 왼쪽 정렬 (anchor='w')
    
    label = tk.Label(feedback_window, text="Please provide your feedback as a number (can include decimals):")
    label.pack(pady=10)
    
    feedback_entry = tk.Entry(feedback_window)  # 텍스트로 소수점 포함 숫자 입력
